...
foreach (@primers) {
	...
	$primerF = substr ($primer_line[3], 0, 10) ;
...
#	my $path = $ARGV[2];
	...
	my $trimmed_R1_file = $prefix."_trimmed_50bps.fastq";		# this is for naming the outfile
	my $Untrimmed_R1_file = $prefix."_greaterThan300_ampl.fastq";
	foreach my $primer_key (sort keys %Primer_hash) {
#		print "$Primer_hash{$key}{Reverse_primer}\t$Primer_hash{$key}{Forward_primer}\n";
	 	if ( ($Primer_hash{$primer_key}{Amplicon_length} =~ /Less_than_300/)  && $reads[$i] =~ /^$Primer_hash{$primer_key}{Reverse_primer}/ ) {    # if the read matches the reverse primer and the amplicon is shorter than 300bps	
			$count = 0 ;
			
			$trimmed_sequence = substr($reads[$i], 0, -51) ; 
			open (TRIMMEDOUTFILE, '>>', $trimmed_R1_file) or die "Can't write to file '$trimmed_R1_file' [$!]\n";
		...
		if ( ($Primer_hash{$primer_key}{Amplicon_length} =~ /Greater_than_300/)  && $reads[$i] =~ /^$Primer_hash{$primer_key}{Reverse_primer}/ ) {    # if the read matches the reverse primer and the amplicon is longer than 300bps	
			$count_2 = 0 ;
			
			open (UNTRIMMEDOUTFILE, '>>', $Untrimmed_R1_file) or die "Can't write to file '$Untrimmed_R1_file' [$!]\n";
